            ...
            if (pris.filter(x => x.sedmica == q).length != 0) {
                for (var zu = 1; zu <= podaci.brojPredavanjaSedmicno; zu++) {
                    if (zu <= pris.filter(x => x.sedmica == q)[0].predavanja)
                        html += "<td onclick=\"modk('"+podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + "," + (pris.filter(x => x.sedmica == q)[0].predavanja - 1) + "," + (pris.filter(x => x.sedmica == q)[0].vjezbe ) + ")\" class=\"prisutan\"> </td>";
                    else
                        html += "<td onclick=\"modk('"+podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + "," + (pris.filter(x => x.sedmica == q)[0].predavanja + 1) + "," + (pris.filter(x => x.sedmica == q)[0].vjezbe ) + ")\" class=\"odsutan\"> </td>";
                }
                for (var zu = 1; zu <= podaci.brojVjezbiSedmicno; zu++) {
                    if (zu <= pris.filter(x => x.sedmica == q)[0].vjezbe)
                        html += "<td onclick=\"modk('"+ podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + "," + (pris.filter(x => x.sedmica == q)[0].predavanja ) + "," + (pris.filter(x => x.sedmica == q)[0].vjezbe - 1) + ")\" class=\"prisutan\"> </td>";
                    else
                        html += "<td onclick=\"modk('"+ podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + "," + (pris.filter(x => x.sedmica == q)[0].predavanja ) + "," + (pris.filter(x => x.sedmica == q)[0].vjezbe + 1) + ")\" class=\"odsutan\"> </td>";
                }
            }
            else {
                for (var zu = 1; zu <= podaci.brojPredavanjaSedmicno; zu++) {

                    html += "<td onclick=\"modk('"+ podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + ",1,0)\"> </td>";
                }
                for (var zu = 1; zu <= podaci.brojVjezbiSedmicno; zu++) {

                    html += "<td onclick=\"modk('"+podaci.predmet+ "'," + q + "," + podaci.studenti[j].index + ",0,1)\"> </td>";
                }
            }
